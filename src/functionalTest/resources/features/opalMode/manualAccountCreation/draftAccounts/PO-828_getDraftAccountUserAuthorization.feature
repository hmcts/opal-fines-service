...
Feature: PO-828 Authorization for get draft account
  ...
  @PO-828 @cleanUpData
  Scenario: Authorization for Test User 5 to Check and Validate Draft Accounts or Create or manage draft accounts
  ...
  @PO-828 @cleanUpData
  Scenario: Authorization for Test User 6 to Check and Validate Draft Accounts or Create or manage draft accounts
  ...
  @PO-828 @cleanUpData
  Scenario: Authorization for Test User 7 to Check and Validate Draft Accounts or Create or manage draft accounts
  ...
  Scenario: Authorization for Test User 8 to Check and Validate Draft Accounts or Create or manage draft accounts
    Given I am testing as the "opal-test-9@HMCTS.NET" user
    When I create a draft account with the following details
      | business_unit_id | 82                                          |
      | account          | draftAccounts/accountJson/adultAccount.json |
      | account_type     | Fine                                        |
    ...
    And The draft account response contains the following data
      ...
      | account_snapshot.account_type       | Fine         |
      | account_snapshot.submitted_by       | BUUID        |
      | account_snapshot.business_unit_name | Bolton       |

    When I get the draft accounts filtering on the Business unit "82" then the response contains
      | business_unit_id                    | 82     |
      | account_snapshot.business_unit_name | Bolton |
