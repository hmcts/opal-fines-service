...
class SummaryResultMapperTest {
    ...
    void shouldMapAccountsToRowsAndPopulateMetadata() {
        ...
        SummaryOperationReportRowDto
            dto1 = new SummaryOperationReportRowDto();
        SummaryOperationReportRowDto
            dto2 = new SummaryOperationReportRowDto();
        ...
        // Act
        OperationSummaryReport result = mapper.map(accounts);
        // Assert
    ...
    void shouldHandleEmptyAccountList() {
        // Act
        OperationSummaryReport result = mapper.map(List.of());
        // Assert
        Assertions.assertThat(result).isNotNull();
        Assertions.assertThat(result.getSummaryReport().getReportSummaryRows()).isEmpty();
    ...
    void shouldHandleNullParticipantsFromContextGracefully() {
        ...
        SummaryOperationReportRowDto
            dto = new SummaryOperationReportRowDto();
        when(rowMapper.map(eq(acc), any())).thenReturn(dto);
        // Act
        OperationSummaryReport result =
            mapper.map(List.of(acc));
        // Assert
        Assertions.assertThat(result.getSummaryReport().getReportSummaryRows()).containsExactly(dto);
        Assertions.assertThat(result.getReportMetaData().getPdpoPartyIds()).isEmpty();
    }
    ...
    void shouldCalculateTotalsCorrectly() {
        ...
        DefendantAccountEntity acc1 = new DefendantAccountEntity();
        DefendantAccountEntity acc2 = new DefendantAccountEntity();
        SummaryOperationReportRowDto row1 = row("A1", "100.00", "50.00", "50.00");
        SummaryOperationReportRowDto row2 = row("A2", "200.00", "100.00", "100.00");
    ...
    void shouldIgnoreNullValuesInTotals() {
        ...
        DefendantAccountEntity acc1 = new DefendantAccountEntity();
        DefendantAccountEntity acc2 = new DefendantAccountEntity();
        SummaryOperationReportRowDto row1 =
            row("A1", null, "50.00", null);
        SummaryOperationReportRowDto row2 =
    ...
    private SummaryOperationReportRowDto row(
        String accountNo,
        String imposed,
        String paid,
        String balance
