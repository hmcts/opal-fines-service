CREATE OR REPLACE PROCEDURE p_stub_create_defendant_account(
	...
	IN pi_posted_by_name  	         varchar, 
	INOUT pio_account_number         draft_accounts.account_number%TYPE,
	INOUT pio_defendant_account_id   draft_accounts.account_id%TYPE
	)
LANGUAGE 'plpgsql'
...
* CGI OPAL Program
*
* MODULE      : p_stub_create_defendant_account.sql
*
* DESCRIPTION : STUB The interface procedure to create manual account. It will parse the account json to insert into the substantive tables.
*
...
*			 : pio_account_number              - The Opal account number to be generated and returned to the backend
*			 : pio_defendant_account_id        - The Opal defendant account id to be generated and returned to the backend
*
* VERSION HISTORY:
*
* Date          Author      Version     Nature of Change
* ----------    -------     --------    -------------------------------------------------------------------------
* 30/06/2025    A Dennis    1.0         PO-1905 STUB. The interface procedure to create manual defendant account.
*
**/
DECLARE
...
BEGIN
	...
	VALUES( 
		  ...
		  , pi_business_unit_id
		  , '25000001X'   --XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX JIRA TICKET PO-899
		  , v_account_type
		  , v_account_sentence_date
	...
	RETURNING 
	      ...
	      , account_number 
	INTO 
	        pio_defendant_account_id
		  , pio_account_number;

EXCEPTION

    WHEN OTHERS THEN
	    RAISE EXCEPTION 'Error in p_stub_create_defendant_account: %',' SQLSTATE: '||SQLSTATE ||'   SQLERRM: '|| SQLERRM;

END;
$BODY$;

COMMENT ON PROCEDURE p_stub_create_defendant_account
    IS 'STUB The interface procedure to create manual account. It will parse the account json to insert into the substantive tables.';