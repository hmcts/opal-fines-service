...
* OPAL Program
*
* MODULE      : alter_p_create_defendant_account_validate_originator.sql
*
* DESCRIPTION : Amend p_create_defendant_account to validate originator_id and originator_name against reference data.
*
* VERSION HISTORY:
*
* Date          Author      Version     Nature of Change
* ----------    -------     --------    -----------------------------------------------------------------------------------------------------------------
* 28/05/2026    C Cho       1.0         PO-5760 Validate originator_id and originator_name against LOCAL_JUSTICE_AREAS or PROSECUTORS.
*
**/

--
-- Name: p_create_defendant_account(draft_accounts.draft_account_id%TYPE, draft_accounts.business_unit_id%TYPE, defendant_transactions.posted_by%TYPE, defendant_transactions.posted_by_name%TYPE); Type: PROCEDURE; Schema: public; Owner: -
--

CREATE OR REPLACE PROCEDURE public.p_create_defendant_account(IN pi_draft_account_id draft_accounts.draft_account_id%TYPE, IN pi_business_unit_id draft_accounts.business_unit_id%TYPE, IN pi_posted_by defendant_transactions.posted_by%TYPE, IN pi_posted_by_name defendant_transactions.posted_by_name%TYPE, OUT po_account_number draft_accounts.account_number%TYPE, OUT po_defendant_account_id draft_accounts.account_id%TYPE)
    LANGUAGE plpgsql
    AS $$
/**
* CGI OPAL Program
...
* 28/05/2026    C Cho       8.0         PO-5760 - Validate originator_id and originator_name against LOCAL_JUSTICE_AREAS or PROSECUTORS.
**/
DECLARE
...
BEGIN
    ...
    -- v8.0 - Validate originator_id and originator_name against LOCAL_JUSTICE_AREAS or PROSECUTORS
    IF NOT EXISTS (
        SELECT 1
          FROM local_justice_areas lja
         WHERE lja.local_justice_area_id::VARCHAR = v_originator_id
           AND lja.name = v_originator_name
        UNION ALL
        SELECT 1
          FROM prosecutors p
         WHERE p.prosecutor_id::VARCHAR = v_originator_id
           AND p.name = v_originator_name
    ) THEN
        RAISE EXCEPTION 'Originator % / % not found', v_originator_id, v_originator_name
            USING ERRCODE = 'P2014'
                , DETAIL = 'p_create_defendant_account: originator_id and originator_name must match LOCAL_JUSTICE_AREAS or PROSECUTORS';
    END IF;
...
EXCEPTION
    WHEN SQLSTATE 'P2002' OR SQLSTATE 'P2003' OR SQLSTATE 'P2004' OR SQLSTATE 'P2005' OR SQLSTATE 'P2006' OR    --v3.0
         SQLSTATE 'P2008' OR SQLSTATE 'P2009' OR SQLSTATE 'P2010' OR SQLSTATE 'P2011' OR SQLSTATE 'P2012' OR SQLSTATE 'P2013' OR SQLSTATE 'P2014' THEN   --v4.0, v8.0
...
--
-- Name: PROCEDURE p_create_defendant_account(IN pi_draft_account_id draft_accounts.draft_account_id%TYPE, IN pi_business_unit_id draft_accounts.business_unit_id%TYPE, IN pi_posted_by defendant_transactions.posted_by%TYPE, IN pi_posted_by_name defendant_transactions.posted_by_name%TYPE, OUT po_account_number draft_accounts.account_number%TYPE, OUT po_defendant_account_id draft_accounts.account_id%TYPE); Type: COMMENT; Schema: public; Owner: -
--

COMMENT ON PROCEDURE public.p_create_defendant_account(IN pi_draft_account_id draft_accounts.draft_account_id%TYPE, IN pi_business_unit_id draft_accounts.business_unit_id%TYPE, IN pi_posted_by defendant_transactions.posted_by%TYPE, IN pi_posted_by_name defendant_transactions.posted_by_name%TYPE, OUT po_account_number draft_accounts.account_number%TYPE, OUT po_defendant_account_id draft_accounts.account_id%TYPE) IS 'The interface procedure to create manual account. It parses the account Json to insert into the substantive tables.';
