...
* CGI OPAL Program
*
* MODULE      : alter_p_add_defendant_account_enforcement_account_controls.sql
*
...
* 17/07/2026    C Cho       2.2         PO-2977 Guard COLLO document creation and stop SC setting suspended_committal_date.
* 22/07/2026    P Brumby    2.3         PO-5756 Add account status and zero balance checks before enforcement creation.
*
**/
DECLARE
    ...
    v_account_status                 t_da_account_status_enum;
    v_account_balance                NUMERIC(18, 2);
...
BEGIN
    ...
    -- Account status and zero balance checks
    SELECT account_status, account_balance
    INTO v_account_status, v_account_balance
    FROM defendant_accounts
    WHERE defendant_account_id = pi_defendant_account_id;

    IF v_account_status IN ('CS', 'TA', 'TO', 'TS', 'WO') THEN
        RAISE EXCEPTION 'Account status is `%`', v_account_status
            USING ERRCODE = 'P4009'
                , DETAIL = 'p_add_defendant_account_enforcement: Account status prevents enforcement creation';
    END IF;

    IF v_account_balance = 0 THEN
        RAISE EXCEPTION 'Account has a zero balance'
            USING ERRCODE = 'P4010'
                , DETAIL = 'p_add_defendant_account_enforcement: Zero account balance prevents enforcement creation';
    END IF;
    ...
    SELECT further_steps_notice_date, fine_registration_date, confiscation_order_date, collection_order
    INTO v_fsn_date, v_fine_reg_date, v_confiscation_date, v_collection_order
    FROM defendant_accounts
    WHERE defendant_account_id = pi_defendant_account_id;
    ...
    FOR v_document_id IN (
        ...
        WHERE result_id = pi_result_id
        AND (v_enforcement_action != 'COLLO' OR COALESCE(v_collection_order, FALSE) = FALSE) --[PO-2977] For COLLO if v_collection_order was FALSE/NULL
                                                                                             --docs are created (because collection_order changed from FALSE/NULL -> TRUE)
    ) LOOP
...
EXCEPTION
    WHEN SQLSTATE 'P4001' OR SQLSTATE 'P4002' OR SQLSTATE 'P4003' OR
         ...
         SQLSTATE 'P4007' OR SQLSTATE 'P4008' OR SQLSTATE 'P4009' OR
         SQLSTATE 'P4010' THEN
