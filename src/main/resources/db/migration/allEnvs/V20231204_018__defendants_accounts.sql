...
*
* DESCRIPTION : Modify the DEFENDANT_ACCOUNTS table to add foreign keys
*
* VERSION HISTORY:
*
* Date          Author      Version     Nature of Change
* ----------    -------     --------    ----------------------------------------------------------------------------
* 05/12/2023    A Dennis    1.0         Modify the DEFENDANT_ACCOUNTS table to add foreign keys 
*
**/

-- DROP and recreate the DEFENDANT_ACCOUNTS table since NULL columns and foreign key relations are now known. Also some column names have changed in the data model
DROP TABLE IF EXISTS defendant_accounts;
...
,enf_override_result_id	            varchar(10)
...
ALTER TABLE defendant_accounts
ADD CONSTRAINT res_enf_override_result_id_fk FOREIGN KEY
(
...
ALTER TABLE defendant_accounts
ADD CONSTRAINT res_enf_override_enforcer_id_fk FOREIGN KEY
(
...
ALTER TABLE defendant_accounts
ADD CONSTRAINT res_enf_override_tfo_lja_id_fk FOREIGN KEY
(
...
COMMENT ON COLUMN defendant_accounts.account_balance IS 'The balance outstanding on this account. Was not stored in Legacy GoB';
...
COMMENT ON COLUMN defendant_accounts.enf_override_result_id IS 'The enforcement result that will be applied if this account is enforcement by auto-enforcement';
COMMENT ON COLUMN defendant_accounts.enf_override_enforcer_id IS 'The enforcer that will be allocated to enforcement override result';
COMMENT ON COLUMN defendant_accounts.enf_override_tfo_lja_id IS 'The ID of the LJA the account will be transferred to if the override result is TFFOUT';
...
COMMENT ON COLUMN defendant_accounts.collection_order_date IS 'The date the collection order status last changed';
...
COMMENT ON COLUMN defendant_accounts.suspended_committal_date IS 'Hearing date when a suspended committal (SC) enforcement result was applied to this account';
