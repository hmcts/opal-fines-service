...
*                                       Taken from: V20250924_416__create_view_v_minor_creditor_account_header.sql
*                                                   Only change: Added DROP statement
* 25/03/2026    C Cho       1.2         PO-3369 - Add has_associated_defendant flag
*
**/
DROP VIEW IF EXISTS v_minor_creditor_account_header;
...
AS 
    SELECT DISTINCT
         ...
         , bu.welsh_language
         , CASE WHEN imp.creditor_account_id IS NOT NULL THEN TRUE ELSE FALSE END AS has_associated_defendant
 ...
 LEFT JOIN ( SELECT DISTINCT creditor_account_id
               FROM impositions
           ) imp
        ON t.creditor_account_id = imp.creditor_account_id;
