...
public class SrvcGetOoooLocked extends TechSpikeAction {
    ...
    public SrvcGetOoooLocked(DraftAccountService draftAccountService, DraftAccountController controller,
                             RestClient restClient, List<Long> draftIds) {
        super(draftAccountService, controller, restClient, 140);
    ...
    public CallResponse doIt() {
        ...
        DraftAccountEntity entity = getDraftAccountEntityOLocked(id, slept);
        long delta = timeMe(start);

        return successCallResponse("Srvc Get O Lock '", entity, delta, slept);
    }
}
