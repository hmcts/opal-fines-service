...
* 26/05/2026    C Cho       3.0         PO-2596 Add amended_by_name audit assertions.
* 17/07/2026    C Cho       4.0         PO-2977 Add COLLO document guard and SC suspended committal date assertions.
* 24/07/2026    P Brumby    5.0         PO-5756 Add account status and zero balance guard assertions.
**/
...
BEGIN
    ...
    DELETE FROM amendments WHERE associated_record_type = 'defendant_accounts' AND associated_record_id IN ('90001', '90002', '90003', '90004', '90005', '90006', '90007', '90008', '90009', '90010', '90011', '90012');
    DELETE FROM report_entries WHERE associated_record_type = 'enforcements' AND associated_record_id IN (SELECT enforcement_id::VARCHAR FROM enforcements WHERE defendant_account_id IN (90001, 90002, 90003, 90004, 90005, 90006, 90007, 90008, 90009, 90010, 90011, 90012));
    DELETE FROM document_instances WHERE associated_record_type = 'enforcements' AND associated_record_id IN (SELECT enforcement_id::VARCHAR FROM enforcements WHERE defendant_account_id IN (90001, 90002, 90003, 90004, 90005, 90006, 90007, 90008, 90009, 90010, 90011, 90012));
    DELETE FROM warrant_register WHERE enforcement_id IN (SELECT enforcement_id FROM enforcements WHERE defendant_account_id IN (90001, 90002, 90003, 90004, 90005, 90006, 90007, 90008, 90009, 90010, 90011, 90012));
    DELETE FROM enforcements WHERE defendant_account_id IN (90001, 90002, 90003, 90004, 90005, 90006, 90007, 90008, 90009, 90010, 90011, 90012);
    DELETE FROM aliases WHERE party_id IN (90001, 90002, 90003, 90004, 90005, 90006, 90007, 90008, 90009, 90010, 90011, 90012);
    DELETE FROM debtor_detail WHERE party_id IN (90001, 90002, 90003, 90004, 90005, 90006, 90007, 90008, 90009, 90010, 90011, 90012);
    DELETE FROM defendant_account_parties WHERE defendant_account_id IN (90001, 90002, 90003, 90004, 90005, 90006, 90007, 90008, 90009, 90010, 90011, 90012);
    DELETE FROM defendant_accounts WHERE defendant_account_id IN (90001, 90002, 90003, 90004, 90005, 90006, 90007, 90008, 90009, 90010, 90011, 90012);
    DELETE FROM parties WHERE party_id IN (90001, 90002, 90003, 90004, 90005, 90006, 90007, 90008, 90009, 90010, 90011, 90012);
...
----------------------------------------------------------------------------------------------------------------------
-- Test 2A: Test COLLO enforcement action does not create documents when collection order already exists
----------------------------------------------------------------------------------------------------------------------
DO LANGUAGE 'plpgsql' $$
DECLARE
    ...
    v_record_count               INTEGER;
BEGIN
    RAISE NOTICE '=== TEST 2A: Test COLLO enforcement action does not create documents when collection order already exists ===';

    -- Setup test data with an existing collection order
    INSERT INTO parties (
        party_id, organisation, surname, forenames, title,
        address_line_1, postcode, birth_date
    ) VALUES (
        90005, FALSE, 'TestDefendant5', 'Clare', 'Ms',
        '555 Test Street', 'TE5 6ST', '1988-06-15'::DATE
    ) RETURNING party_id INTO v_party_id_def;
    ...
    INSERT INTO defendant_accounts (
        ...
        v_defendant_account_id, v_business_unit_id, 'TEST005', 'Fine',
        '2024-01-01'::DATE, 250.00, 0.00, 250.00,
        'L', TRUE, '2024-01-15 10:00:00'::TIMESTAMP, 1
    );

    INSERT INTO defendant_account_parties (
        defendant_account_party_id, defendant_account_id, party_id, association_type, debtor
    ) VALUES (90005, v_defendant_account_id, v_party_id_def, 'Defendant', FALSE);

    -- Verify COLLO result exists and has a configured document to guard against duplicate generation
    SELECT COUNT(*) INTO v_expected_doc_count
    FROM result_documents
    WHERE result_id = v_result_id;

    ASSERT v_expected_doc_count > 0, 'COLLO should have configured result documents for this test';
    ...
    -- Verify no duplicate COLLO document instances were created
    SELECT COUNT(*) INTO v_record_count FROM document_instances
    WHERE associated_record_type = 'enforcements' AND associated_record_id = v_enforcement_id::VARCHAR;

    ASSERT v_record_count = 0, 'COLLO should not create document instances when collection order already exists';

    RAISE NOTICE 'TEST 2A PASSED: COLLO enforcement did not create duplicate collection order documents';
END $$;

----------------------------------------------------------------------------------------------------------------------
-- Test 2B: Test SC enforcement action does not set suspended committal date
----------------------------------------------------------------------------------------------------------------------
DO LANGUAGE 'plpgsql' $$
DECLARE
    v_defendant_account_id       BIGINT := 90006;
    v_business_unit_id           SMALLINT := 65;
    v_result_id                  VARCHAR(6) := 'SC';
    v_party_id_def               BIGINT;
    v_enforcement_id             BIGINT;
    v_jail_days                  INTEGER := 45;
BEGIN
    RAISE NOTICE '=== TEST 2B: Test SC enforcement action does not set suspended committal date ===';

    -- Setup test data without a suspended committal date
    INSERT INTO parties (
        party_id, organisation, surname, forenames, title,
        address_line_1, postcode, birth_date
    ) VALUES (
        90006, FALSE, 'TestDefendant6', 'David', 'Mr',
        '666 Test Street', 'TE6 7ST', '1979-07-20'::DATE
    ) RETURNING party_id INTO v_party_id_def;

    INSERT INTO defendant_accounts (
        defendant_account_id, business_unit_id, account_number, account_type,
        imposed_hearing_date, amount_imposed, amount_paid, account_balance,
        account_status, suspended_committal_date, jail_days, version_number
    ) VALUES (
        v_defendant_account_id, v_business_unit_id, 'TEST006', 'Fine',
        '2024-01-01'::DATE, 175.00, 0.00, 175.00,
        'L', NULL, 0, 1
    );

    INSERT INTO defendant_account_parties (
        defendant_account_party_id, defendant_account_id, party_id, association_type, debtor
    ) VALUES (90006, v_defendant_account_id, v_party_id_def, 'Defendant', FALSE);

    -- Verify SC result exists
    PERFORM 1 FROM results WHERE result_id = v_result_id AND enforcement = TRUE AND active = TRUE;
    IF NOT FOUND THEN
        RAISE EXCEPTION 'SC result not found in test data';
    END IF;
    ...
    -- Verify SC does not set suspended_committal_date
    PERFORM 1 FROM defendant_accounts
    WHERE defendant_account_id = v_defendant_account_id
      AND last_enforcement = v_result_id
      AND jail_days = v_jail_days
      AND suspended_committal_date IS NULL;

    ASSERT FOUND, 'SC enforcement should update enforcement details without setting suspended_committal_date';

    RAISE NOTICE 'TEST 2B PASSED: SC enforcement did not set suspended committal date';
END $$;
...
----------------------------------------------------------------------------------------------------------------------
-- Test 10: Test error handling - restricted account statuses prevent enforcement creation
----------------------------------------------------------------------------------------------------------------------
DO LANGUAGE 'plpgsql' $$
DECLARE
    v_business_unit_id           SMALLINT := 65;
    v_statuses                   TEXT[] := ARRAY['CS', 'TA', 'TO', 'TS', 'WO'];
    v_status                     TEXT;
    v_defendant_account_id       BIGINT;
    v_party_id_def               BIGINT;
    v_error_caught               BOOLEAN;
    v_expected_sqlstate          VARCHAR := 'P4009';
    v_expected_message           VARCHAR;
    v_enforcement_id             BIGINT;
    v_record_count               INTEGER;
BEGIN
    RAISE NOTICE '=== TEST 10: Test error handling - restricted account statuses prevent enforcement creation ===';

    FOR v_status IN SELECT unnest(v_statuses) LOOP
        v_defendant_account_id := 90006 + array_position(v_statuses, v_status);
        v_error_caught := FALSE;
        v_expected_message := format('Account status is `%s`', v_status);
        v_enforcement_id := NULL;

        INSERT INTO parties (
            party_id, organisation, surname, forenames, address_line_1, postcode, birth_date
        ) VALUES (
            v_defendant_account_id, FALSE, format('TestDefendant%s', v_status), 'Restricted',
            format('%s Status Street', v_status), 'TE7 8ST', '1981-08-21'::DATE
        ) RETURNING party_id INTO v_party_id_def;
        ...
        INSERT INTO defendant_accounts (
            ...
            v_defendant_account_id, v_business_unit_id, format('TEST%s', v_defendant_account_id), 'Fine',
            '2024-01-01'::DATE, 300.00, 0.00, 300.00,
            v_status::t_da_account_status_enum, 1
        );

        INSERT INTO defendant_account_parties (
            defendant_account_party_id, defendant_account_id, party_id, association_type, debtor
        ) VALUES (v_defendant_account_id, v_defendant_account_id, v_party_id_def, 'Defendant', FALSE);
        ...
        BEGIN
            CALL p_add_defendant_account_enforcement(
                ...
                pi_posted_by := 'USER123',
                pi_posted_by_name := 'Test User',
                pi_reason := 'Test restricted account status',
                pi_enforcer_id := NULL,
                pi_result_responses := NULL,
                pi_earliest_release_date := NULL,
                pi_version_number := 1,
        ...
        EXCEPTION
            WHEN SQLSTATE 'P4009' THEN
                IF SQLERRM = v_expected_message THEN
                    v_error_caught := TRUE;
                    RAISE NOTICE 'Expected error caught for status %: % - %', v_status, SQLSTATE, SQLERRM;
                ELSE
                    RAISE WARNING 'Expected error SQLSTATE caught for status % but with wrong SQLERRM: % - %', v_status, SQLSTATE, SQLERRM;
                END IF;
            WHEN OTHERS THEN
                v_error_caught := FALSE;
                RAISE NOTICE 'Unexpected error caught for status %: % - %', v_status, SQLSTATE, SQLERRM;
        END;

        ASSERT v_error_caught = TRUE, format('A %s error should have been raised for account_status %s', v_expected_sqlstate, v_status);

        SELECT COUNT(*) INTO v_record_count
        FROM enforcements
        WHERE defendant_account_id = v_defendant_account_id;

        ASSERT v_record_count = 0, format('No enforcement should be created for account_status %s', v_status);

        PERFORM 1 FROM defendant_accounts
        WHERE defendant_account_id = v_defendant_account_id
          AND version_number = 1
          AND last_enforcement IS NULL;

        ASSERT FOUND, format('Defendant account should not be updated for account_status %s', v_status);
    END LOOP;

    RAISE NOTICE 'TEST 10 PASSED: Restricted account statuses correctly prevent enforcement creation';
END $$;

----------------------------------------------------------------------------------------------------------------------
-- Test 11: Test error handling - zero account balance prevents enforcement creation
----------------------------------------------------------------------------------------------------------------------
DO LANGUAGE 'plpgsql' $$
DECLARE
    v_defendant_account_id       BIGINT := 90012;
    v_business_unit_id           SMALLINT := 65;
    v_party_id_def               BIGINT;
    v_error_caught               BOOLEAN := FALSE;
    v_expected_sqlstate          VARCHAR := 'P4010';
    v_expected_message           VARCHAR := 'Account has a zero balance';
    v_enforcement_id             BIGINT;
BEGIN
    RAISE NOTICE '=== TEST 11: Test error handling - zero account balance prevents enforcement creation ===';

    INSERT INTO parties (
        party_id, organisation, surname, forenames, address_line_1, postcode, birth_date
    ) VALUES (
        v_defendant_account_id, FALSE, 'TestDefendantZeroBalance', 'Zoe',
        '1212 Test Street', 'TE12 3ST', '1982-09-22'::DATE
    ) RETURNING party_id INTO v_party_id_def;
    ...
    INSERT INTO defendant_accounts (
        ...
        v_defendant_account_id, v_business_unit_id, 'TEST012', 'Fine',
        '2024-01-01'::DATE, 300.00, 300.00, 0.00,
        'L', 1
    );
    ...
    BEGIN
        CALL p_add_defendant_account_enforcement(
            ...
            pi_posted_by := 'USER123',
            pi_posted_by_name := 'Test User',
            pi_reason := 'Test zero account balance',
            pi_enforcer_id := NULL,
            pi_result_responses := NULL,
            pi_earliest_release_date := NULL,
            pi_version_number := 1,
    ...
    ASSERT v_error_caught = TRUE, format('A %s error should have been raised due to zero account balance', v_expected_sqlstate);

    PERFORM 1 FROM enforcements WHERE defendant_account_id = v_defendant_account_id;
    ASSERT NOT FOUND, 'No enforcement should be created when account balance is zero';
    ...
    ASSERT FOUND, 'Defendant account should not be updated when account balance is zero';

    RAISE NOTICE 'TEST 11 PASSED: Zero account balance correctly prevents enforcement creation';
END $$;
