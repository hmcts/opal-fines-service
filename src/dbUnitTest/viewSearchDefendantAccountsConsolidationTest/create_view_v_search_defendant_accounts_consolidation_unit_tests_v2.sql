...
* 17/07/2026    P Brumby    2.0         PO-5740 - Add TO and TS account status error coverage
*
**/
...
BEGIN
    ...
    DELETE FROM defendant_account_parties WHERE defendant_account_id IN (7701, 7702, 7703, 7704, 7705, 7706);
    DELETE FROM defendant_accounts WHERE defendant_account_id IN (7701, 7702, 7703, 7704, 7705, 7706);
    DELETE FROM parties WHERE party_id IN (7701, 7702, 7703, 7704, 7705, 7706);
...
DECLARE
    ...
    v_account_to_status      bigint := 7705;
    v_account_ts_status      bigint := 7706;
BEGIN
    ...
    (7705, FALSE, NULL, 'Transfer', 'Tom', 'Mr', '5 Transfer Terrace', 'TO1 1TO', '1993-05-05'),
    (7706, FALSE, NULL, 'Suspense', 'Tina', 'Ms', '6 Suspense Street', 'TS1 1TS', '1994-06-06')
    ...
    (v_account_to_status, 77, 'CON-TO-01', 250.00, 0.00, 250.00, 'TO', 'Fine', 'PCR-TO', NULL, NULL, NULL, FALSE, 1, NULL),
    (v_account_ts_status, 77, 'CON-TS-01', 260.00, 0.00, 260.00, 'TS', 'Fine', 'PCR-TS', NULL, NULL, NULL, FALSE, 1, NULL)
    ...
    (7705, v_account_to_status, 7705, 'Defendant', TRUE),
    (7706, v_account_ts_status, 7706, 'Defendant', TRUE)
    ...
    RAISE NOTICE 'Test data setup completed for accounts %, %, %, %, %, %', v_account_error, v_account_warning, v_account_clean, v_account_cleared, v_account_to_status, v_account_ts_status;
END $$;
...
-- Test 5: Account status TO generates account status error
DO $$
DECLARE
    v_errors         text[];
    v_warnings       text[];
BEGIN
    RAISE NOTICE '--- TEST 5: Account status TO generates account status error ---';

    SELECT errors, warnings
    INTO v_errors, v_warnings
    FROM v_search_defendant_accounts_consolidation
    WHERE defendant_account_id = 7705;

    ASSERT v_errors = ARRAY[
        'CON.ER.1|Account status is `TO`'
    ], 'Expected CON.ER.1 error for account status TO';

    ASSERT v_warnings IS NULL, 'Warnings should be NULL when only TO account status error condition applies';

    RAISE NOTICE 'TEST 5 PASSED';
END $$;

-- Test 6: Account status TS generates account status error
DO $$
DECLARE
    v_errors         text[];
    v_warnings       text[];
BEGIN
    RAISE NOTICE '--- TEST 6: Account status TS generates account status error ---';

    SELECT errors, warnings
    INTO v_errors, v_warnings
    FROM v_search_defendant_accounts_consolidation
    WHERE defendant_account_id = 7706;

    ASSERT v_errors = ARRAY[
        'CON.ER.1|Account status is `TS`'
    ], 'Expected CON.ER.1 error for account status TS';

    ASSERT v_warnings IS NULL, 'Warnings should be NULL when only TS account status error condition applies';
