...
* 06/03/2026    T McCallion    2.0         PO-2846 Update local_justice_areas.lja_type value to valid ENUM value (Magistrates to CRWCRT)
* 17/08/2026    P Brumby       3.0         PO-9756 Add regression test for defendant account with removed Defendant party association
*
**/
...
DO $$
BEGIN
    RAISE NOTICE '=== Start tests for v_audit_defendant_accounts view ===';
    ...
    DELETE FROM defendant_accounts WHERE defendant_account_id IN (999801, 999802, 999803, 999804);
...
-- Test 4: Defendant account remains visible after Defendant party association is removed
DO $$
DECLARE
    v_defendant_account_id      bigint := 999804;
    v_row_count                 integer;
    v_account_comments          text;
    v_name                      varchar(255);
    v_birth_date                timestamp;
    ...
    v_document_language         varchar(2);
BEGIN
    RAISE NOTICE '=== TEST 4: Defendant account remains visible without Defendant party association ===';
    ...
    INSERT INTO defendant_accounts (
        ...
        'L',
        'Fine',
        'PO-9756 account with removed Defendant party association',
        999801
    );
    ...
    ASSERT v_row_count = 1, 'View should return one row for defendant account with no Defendant party association';

    SELECT account_comments,
           name,
           birth_date,
           age,
           address_line_1,
    ...
    ASSERT v_account_comments = 'PO-9756 account with removed Defendant party association',
        'Account-level fields should still be returned';

--  Returned v_name is an empty string, not converted to NULL, in line with pname field defined in view.
    ASSERT v_name = '', 'Defendant name should be an empty string when Defendant party association is absent';

    ASSERT v_birth_date IS NULL, 'Defendant birth_date should be NULL when Defendant party association is absent';
    ASSERT v_age IS NULL, 'Defendant age should be NULL when Defendant party association is absent';
    ASSERT v_address_line_1 IS NULL, 'Defendant address_line_1 should be NULL when Defendant party association is absent';
    ASSERT v_address_line_2 IS NULL, 'Defendant address_line_2 should be NULL when Defendant party association is absent';
    ASSERT v_address_line_3 IS NULL, 'Defendant address_line_3 should be NULL when Defendant party association is absent';
    ASSERT v_postcode IS NULL, 'Defendant postcode should be NULL when Defendant party association is absent';
    ASSERT v_national_insurance_number IS NULL, 'NI number should be NULL when Defendant party association is absent';
    ASSERT v_telephone_home IS NULL, 'Home telephone should be NULL when Defendant party association is absent';
    ASSERT v_telephone_business IS NULL, 'Business telephone should be NULL when Defendant party association is absent';
    ASSERT v_telephone_mobile IS NULL, 'Mobile telephone should be NULL when Defendant party association is absent';
    ASSERT v_email_1  IS NULL, 'Primary email should be NULL when Defendant party association is absent';
    ASSERT v_email_2  IS NULL, 'Secondary email should be NULL when Defendant party association is absent';
    ASSERT v_alias1 IS NULL, 'Alias fields should be NULL when Defendant party association is absent';
    ASSERT v_document_language IS NULL, 'Debtor detail fields should be NULL when Defendant party association is absent';
  

    RAISE NOTICE 'TEST 4 PASSED: Defendant account remains visible with NULL party-derived fields';
END $$;
