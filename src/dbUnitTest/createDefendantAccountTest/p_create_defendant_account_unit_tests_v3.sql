...
DECLARE
    ...
    v_defendant_account_id   BIGINT;
BEGIN
    RAISE NOTICE '=== TEST 2: Basic account creation with minimum required fields and account_type = "Fixed Penalty", payment_terms "B" with effective_date and jail_days';
    ...
    -- Verify the payment_terms record for the defendant was created as expected and 28 days have been added to effective_date   v3.0
    ASSERT EXISTS (
        SELECT 1 FROM payment_terms WHERE defendant_account_id = v_defendant_account_id 
                                      AND terms_type_code = 'B' AND effective_date = '2025-01-29' AND active = TRUE 
                                      AND account_balance = -50
    ), 'Defendant payment_terms record should exist in the database';
...
BEGIN 
    ...
    ASSERT (SELECT terms_type_code FROM payment_terms WHERE defendant_account_id = v_defendant_account_id) = 'I', 'PAYMENT_TERMS - terms_type_code should match';
    ASSERT (SELECT effective_date FROM payment_terms WHERE defendant_account_id = v_defendant_account_id) = '2025-01-29', 'PAYMENT_TERMS - effective_date should match';
...
--         8B: When payment_terms_type_code = B, effective_date IS NOT NULL but account_type is not 'Fixed Penalty'
