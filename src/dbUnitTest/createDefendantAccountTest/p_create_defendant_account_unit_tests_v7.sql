...
* 16/03/2026    TMc         6.0         PO-2930 - Amended value for DOCUMENT_INSTANCES.ASSOCIATED_RECORD_TYPE, from defendant_account to defendant_accounts
*                                       PO-2868 - Amended test 5 to check for invalid ENUM value for account_type. Replaces the check constraint violation test
*                                       Amended test 3, updated enforcements.account_type column to enforcements.enforcement_account_type as the column was renamed.
* 28/05/2026    C Cho       7.0         PO-5760 - Updated originator test data to use a valid LOCAL_JUSTICE_AREAS record and added invalid originator validation coverage.
**/
    ...
    ASSERT EXISTS (
        SELECT 1 FROM document_instances
                WHERE associated_record_id = v_defendant_account_id::VARCHAR AND business_unit_id = v_business_unit_id
                  AND associated_record_type = 'defendant_accounts'  AND status = 'New'
...
BEGIN 
    ...
    LOOP
        ...
        ASSERT v_record.enforcement_account_type IS NULL, FORMAT('ENFORCEMENTS - Row %s - enforcement_account_type should be NULL', v_record.row_num);
    ...
    ASSERT (SELECT associated_record_type FROM document_instances WHERE associated_record_id = v_defendant_account_id::VARCHAR AND document_id = 'CY_FINOR') = 'defendant_accounts'
    ...
    ASSERT (SELECT associated_record_type FROM document_instances WHERE associated_record_id = v_defendant_account_id::VARCHAR AND document_id = 'FINOTA') = 'defendant_accounts'
...
----------------------------------------------------------------------------------------------------------------------
-- Test 5: Test handling an invalid account_type ENUM value
----------------------------------------------------------------------------------------------------------------------
DO LANGUAGE 'plpgsql' $$
DECLARE
    ...
    v_row_count                 INTEGER;
    v_expected_errmsg           VARCHAR     := 'Error in p_create_defendant_account: 22P02 - invalid input value for enum t_da_account_type_enum: "fine"';
BEGIN
    RAISE NOTICE '=== TEST 5: Test handling invalid account_type ENUM value ===';
    ...
    -- Verify error was caught
    ASSERT v_error_caught = TRUE, 'Test 5: A 22P02 error, with correct SQLERRM, should have been raised due to invalid account type';
    ...
    RAISE NOTICE 'TEST 5 PASSED: Error handling works correctly when the passed account type is invalid.';
END $$;
...
-------------------------------------------------------------------------------------------------------------------
-- Test 18: Test handling when originator_id and originator_name do not match LOCAL_JUSTICE_AREAS or PROSECUTORS
--          Expected exception: P2014 Originator <ID> / <NAME> not found
-------------------------------------------------------------------------------------------------------------------
DO LANGUAGE 'plpgsql' $$
DECLARE
    v_draft_account_id        BIGINT     := 10021;
    v_business_unit_id        SMALLINT   := 65;
    v_posted_by               VARCHAR    := 'L045EO';
    v_posted_by_name          VARCHAR    := 'Tester 18';
    v_account_json            JSON;
    v_account_number          VARCHAR;
    v_defendant_account_id    BIGINT;
    v_error_caught            BOOLEAN    := FALSE;
    v_expected_errmsg         VARCHAR    := 'Originator 99999 / Invalid Originator not found';
    v_row_count               INTEGER;
BEGIN
    RAISE NOTICE '=== TEST 18: Test handling when the passed originator is invalid ===';

    v_account_json := '{
        "account_type": "Fine",
        "defendant_type": "adultOrYouthOnly",
        "originator_name": "Invalid Originator",
        "originator_id": "99999",
        "originator_type": "TFO",
        "enforcement_court_id": 650000000045,
        "payment_card_request": true,
    ...
    EXCEPTION
        WHEN SQLSTATE 'P2014' THEN
            IF SQLERRM = v_expected_errmsg THEN
                v_error_caught := TRUE;
                RAISE NOTICE 'Test 18: Expected error caught: % - %', SQLSTATE, SQLERRM;
            ELSE
                RAISE WARNING 'Test 18: Expected error SQLSTATE caught but with wrong SQLERRM: % - %', SQLSTATE, SQLERRM;
            END IF;
        WHEN OTHERS THEN
            v_error_caught := FALSE;
            RAISE NOTICE 'Test 18: Unexpected error caught: % - %', SQLSTATE, SQLERRM;
    END;

    ASSERT v_error_caught = TRUE, 'Test 18: A P2014 error, with correct SQLERRM, should have been raised due to invalid originator data';

    SELECT COUNT(*)
      INTO v_row_count
      FROM defendant_accounts
     WHERE business_unit_id = v_business_unit_id
       AND originator_id = '99999'
       AND originator_name = 'Invalid Originator';

    ASSERT v_row_count = 0, 'Test 18: An error was caught but a DEFENDANT_ACCOUNTS row was still created';

    RAISE NOTICE 'TEST 18 PASSED: Error handling works correctly when the passed originator is invalid';
