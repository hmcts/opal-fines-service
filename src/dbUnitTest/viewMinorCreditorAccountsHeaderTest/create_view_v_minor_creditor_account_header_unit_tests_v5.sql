...
*                                           PO-2869 - parties.account_type
*                                           PO-2908 - creditor_accounts.creditor_account_type
*                                           PO-2868 - defendant_accounts.account_status
*                                       PO-2291 - Column originator_name was dropped from impositions table
* 25/03/2026    C Cho       3.0         PO-3369 - Add has_associated_defendant assertions
* 08/07/2026    P Brumby    4.0         PO-7421 - Add business_unit_code assertions for v_minor_creditor_account_header
* 15/07/2026    C Cho       5.0         PO-2975 - Add repayment creditor scenario (awarded/awaiting/outstanding = 0,
*                                                 paid_out = earliest creditor_transaction.transaction_amount)
**/
...
DECLARE
    ...
    v_has_associated_defendant    BOOLEAN;
...
BEGIN
    ...
    -- Scenario 6 - Repayment creditor - awarded/awaiting/outstanding = 0, paid_out = earliest txn amount
    ...
    -- Repayment creditor party (Scenario 6)
    INSERT INTO parties
            (
                party_id,
                organisation,
    ...
    INSERT INTO cheques
            (
                ...
                (3, 1024, 6666, (CURRENT_TIMESTAMP - INTERVAL '30 days'), 3, NULL,11000, 'COMP', (CURRENT_TIMESTAMP + INTERVAL '30 days'), 'W');
    ...
    -- Insert test data - Creditor Accounts - Scenario 6 (Repayment creditor)
    INSERT INTO creditor_accounts
            (
                creditor_account_id,
                business_unit_id,
    ...
    -- Scenario 6 - Multiple creditor transactions; view should pick the earliest by posted_date
    INSERT INTO creditor_transactions
            (
                creditor_transaction_id,
                creditor_account_id,
    ...
    FOR i IN 1..6 LOOP
        ...
        CASE i
            ...
            WHEN 6 THEN
                ...
                -- returns the earliest creditor_transaction.transaction_amount (2500).
                ASSERT v_surname = 'Payback', 'Surname should be Payback';
                ASSERT v_awarded = 0, 'Awarded amount should be 0 for repayment creditor';
                ASSERT v_paid_out = 2500, 'Paid out should be earliest transaction amount (2500) for repayment creditor';
                ASSERT v_awaiting_payment = 0, 'Awaiting payment should be 0 for repayment creditor';
