...
public class MasterDefendantAccountsApiIntegrationTest extends AbstractOpalDefendantsIntegrationTest {
    ...
    private static final long DEFENDANT_ACCOUNT_ID = 233300L;
    private static final long DEFENDANT_ACCOUNT_ID_NO_MASTER = 2006L;
    ...
    void getDefendantAccountMaster_whenAccountExists_returnsMasterAccountIdWithEtag() throws Exception {
        mockMvc.perform(get(URL.formatted(DEFENDANT_ACCOUNT_ID))
            ...
            .andExpect(jsonPath("$.defendant_account_id").value(DEFENDANT_ACCOUNT_ID));
    }
    ...
    @Test
    @DisplayName("returns 404 when the master defendant account cannot be loaded")
    void getDefendantAccountMaster_whenMasterAccountMissing_returnsNotFound() throws Exception {
        mockMvc.perform(get(URL.formatted(DEFENDANT_ACCOUNT_ID_NO_MASTER))
                .with(userStateStub.getAuthenticaitonRequestPostProcessor())
                .header(AUTHORIZATION, userStateStub.getBearerToken()))
            .andExpect(status().isNotFound())
            .andExpect(content().contentType(MediaType.APPLICATION_PROBLEM_JSON))
