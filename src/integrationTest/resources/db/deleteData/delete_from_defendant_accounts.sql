...
DELETE FROM defendant_account_parties WHERE defendant_account_party_id IN (77, 88, 901, 333, 555, 666, 777, 444, 999, 10001, 10002, 10003, 10004, 77444);

-- Remove from fixed_penalty_offences
DELETE FROM fixed_penalty_offences WHERE defendant_account_id IN (77, 88, 901, 333, 555, 666, 777, 444, 999, 77444);

-- Remove payment_terms
DELETE FROM payment_terms WHERE defendant_account_id IN (77, 88, 901, 333, 555, 666, 777, 444, 999, 10001, 10002, 10003, 10004, 77444);

-- Remove notes (ASSOCIATED_RECORD_ID is varchar, so use quoted strings!)
DELETE FROM notes WHERE associated_record_id IN ('77', '88', '901', '333', '555', '666', '777', '444', '999', '77444');

-- Remove main defendant accounts
DELETE FROM defendant_accounts WHERE defendant_account_id IN (77, 88, 901, 333, 555, 666, 777, 444, 999, 10001, 10002, 10003, 10004, 77444);
