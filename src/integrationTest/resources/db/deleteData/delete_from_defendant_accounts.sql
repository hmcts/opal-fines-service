...
DELETE FROM aliases WHERE alias_id IN (8801, 9011, 5551, 5552, 6661, 6662, 7771, 10001);
...
DELETE FROM defendant_account_parties WHERE defendant_account_party_id IN (77, 88, 901, 333, 555, 666, 777, 444, 999, 10001);
...
DELETE FROM payment_terms WHERE defendant_account_id IN (77, 88, 901, 333, 555, 666, 777, 444, 999, 10001);
...
DELETE FROM defendant_accounts WHERE defendant_account_id IN (77, 88, 901, 333, 555, 666, 777, 444, 999, 10001);

-- Remove from debtor_detail before removing parties
DELETE FROM debtor_detail WHERE party_id IN (77, 88, 901, 333, 555, 666, 777, 444, 999, 10001);

-- Remove inserted parties
DELETE FROM parties WHERE party_id IN (77, 88, 901, 333, 555, 666, 777, 444, 999, 10001);
